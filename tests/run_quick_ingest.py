...
def build_metadata(fpath, collection_label, chunk_meta=None):
...
def ingest_directory(collection_map, directory, pattern, collection_label, description,
    ...
    for i, fpath in enumerate(files, 1):
        try:
            ...
            if not skip_fingerprint and store and has_fingerprint(store, fp):
                skipped += 1
                continue
            ...
            for chunk_text, chunk_meta in chunks_with_meta:
                if not chunk_text.strip():
                    continue
                meta = build_metadata(fpath, collection_label, chunk_meta)
                meta["fingerprint"] = fp
            ...
            if texts:
                ...
                if i % 25 == 0 or i == len(files) or i <= 3:
                    extra = f" (retried {retry_splits})" if retry_splits else ""
                    print(f"    [{i}/{len(files)}] {fpath.name}: {ingested} chunks{extra}")
            else:
                skipped += 1
    ...
    print(f"      Result: {success} files ({total_chunks} chunks), {skipped} skipped, {errors} errors")
